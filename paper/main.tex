...
\paragraph{Positioning relative to alternative approaches.}
Table~\ref{tab:comparison} summarizes the core differences between five common approaches to large-move estimation and the system proposed here. Each alternative targets a related but distinct quantity; our contribution is not in any individual component (GARCH, MC, Platt, histogram binning are all well-established) but in combining them into a closed loop that is audited against the four properties most often missing from published large-move work: (i)~strict no-lookahead through a resolution queue, (ii)~overlap-corrected statistics (residual-based $N_\text{eff}$, circular block bootstrap, FDR correction), (iii)~hard promotion gates enforced by a \emph{constrained} Bayesian optimizer (Section~\ref{sec:bo}), and (iv)~a tamper-evident append-only ledger that lets third parties recompute every metric from raw forecast logs. Quantitative head-to-head Brier comparisons against seven implemented baselines with FDR-adjusted paired block-bootstrap tests are reported in Section~\ref{sec:baselines}.

\begin{table}[t]
\centering
\small
\caption{Comparison of common approaches to large-move probability estimation. ``Calibration'' refers to whether the method learns to correct its own probabilistic output against realized outcomes; ``overlap-aware'' refers to statistical tests that correct for autocorrelation induced by overlapping $H$-step predictions; ``live-auditable'' refers to an append-only forecast ledger with cryptographic anchoring.}
\label{tab:comparison}
\begin{tabular}{lccccc}
\toprule
Approach & Target & Calibration & Walk-forward & Overlap-aware & Live-auditable \\
\midrule
Black--Scholes / implied vol & Risk-neutral & implicit & n/a & no & n/a \\
GARCH + Gaussian/$t$ CDF & Physical & none & partial & rarely & rarely \\
HMM / regime-switching & Regime-cond. & ad hoc & partial & rarely & rarely \\
Feature ML (logistic, GBM) & Physical & train-set & possible & rarely & rarely \\
VaR / CVaR models & Loss quantile & coverage test & yes & partial & rarely \\
\midrule
\textbf{This work} & Physical & 3-layer online & strict & yes & yes \\
\bottomrule
\end{tabular}
\end{table}
